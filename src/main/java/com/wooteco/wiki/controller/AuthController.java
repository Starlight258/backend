...
import com.wooteco.wiki.dto.RefreshTokenRequest;
...
public class AuthController {
    ...
    @PostMapping("/refresh")
    @Operation(description = "토큰 재발급 API. 응답으로 엑세스 토큰과 리프레쉬 토큰을 반환한다. 쿠키에 httpOnly 엑세스 토큰이 저장되어있다.")
    @ErrorApiResponse({MEMBER_NOT_FOUNT, TOKEN_INVALID})
    @ApiSuccessResponse(bodyType = RefreshTokenRequest.class, body = """
            {
                "refreshToken": "eyJhbGciOiJIUzUxMiJ9.eyJleHAiOjE3MTgwODUxODAsInRva2VuX3R5cGUiOiJyZWZyZXNoIiwibWVtYmVyX2lkIjoyfQ.MozmvIX38mBfjzVwA2ehBat9hrizUIKzNbh2hDozQeDwLVxgaH0KcHzBic3pvLHINaYOvf2TpH9q4vUKa9QR4w"
            }
            """)
    public ResponseEntity<AuthTokens> refresh(@RequestBody RefreshTokenRequest refreshTokenRequest) {
        AuthTokens authTokens = authService.refresh(refreshTokenRequest.refreshToken());
        return makeResponseEntityWithTokenCookie(authTokens);
    }
}
